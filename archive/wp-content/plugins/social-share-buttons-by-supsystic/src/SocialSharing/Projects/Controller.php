...
class SocialSharing_Projects_Controller extends SocialSharing_Core_BaseController
{
    ...
    public function saveAction(Rsc_Http_Request $request)
    {
		...
		if($settings['where_to_show'] != 'membership') {
			$this->getEnvironment()->getModule('membership')->disableSocialSharing($id);
		}
    ...
    public function viewAction(Rsc_Http_Request $request)
    {
		...
		$memberShipModule = $this->getEnvironment()->getModule('membership');
        ...
        return $this->response(
            ...
            array(
				...
				'membership_installed' => $memberShipModule->isMemberShipActivated(),
				'membership_install_url' => $memberShipModule->getPluginInstallUrl(),
